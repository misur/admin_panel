...
(deftest test-app
  (testing "get all users"
   (let [ response (app-routes {:uri "/users" :request-method :get})]
  ...
  (testing "insert new user"
   (let [ response (app-routes {:uri "/users" :request-method :post :params {:username "test22" :password "test22" :type 2 :email "test22"}})]
     (is (= (:status response) 200))
     (is (= (get-in response [:headers "Content-Type"]) "application/json"))))


  (testing "insert new user"
   (let [ response (app-routes {:uri "/users/1" :request-method :put :params {:username "test22" :password "test22" :type 2 :email "test22"}})]
     (is (= (:status response) 200))
     (is (= (get-in response [:headers "Content-Type"]) "application/json")))))
